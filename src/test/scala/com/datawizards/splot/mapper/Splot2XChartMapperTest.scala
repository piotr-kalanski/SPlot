...
import org.knowm.xchart.style.{MatlabTheme, Styler}
import org.scalatest.FunSuite
import org.scalatest.junit.JUnitRunner

@RunWith(classOf[JUnitRunner])
class Splot2XChartMapperTest extends FunSuite {
  val plot: Plot = buildExamplePlot(PlotType.Bar)
  val chart: Chart[_ <: Styler, _ <: Series]  = SPlotToXChartMapper.mapPlotToXChart(plot)
  val barChart: CategoryChart = chart.asInstanceOf[CategoryChart]
  ...
  test("Annotations") {
    assert(chart.getStyler.hasAnnotations)
  }
